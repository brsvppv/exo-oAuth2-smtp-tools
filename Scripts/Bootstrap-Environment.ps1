...
.DESCRIPTION
  Installs required PowerShell modules, ensures PowerShell 7+ is used, creates a copy of
  `config/smtp-app.example.json` at `config/smtp-app.json` for editing, and prints
  recommended next steps for running `New-ExoOauthSmtpAppIdentity`.
#>
...
function Install-ModuleIfMissing {
    ...
    } else { Write-Log "Module $Name already present" 'INFO' }
} 

if ($PSVersionTable.PSVersion.Major -lt 7) {
    Write-Warning "PowerShell 7+ is recommended. Consider installing from https://aka.ms/powershell"
}
...
if (-not (Test-Path $dest)) {
    ...
    Write-Log "Created config/smtp-app.json from example. Edit it to set your tenant, displayname, and mailboxes." 'INFO'
} else {
    Write-Log "Config file already exists at config/smtp-app.json" 'INFO'
}
...
Write-Log "       iex (irm 'https://raw.githubusercontent.com/<owner>/<repo>/main/Scripts/New-ExoOauthSmtpAppIdentity_v3.ps1')" 'INFO'
...
Write-Log "       pwsh .\Scripts\New-ExoOauthSmtpAppIdentity_v3.ps1; New-ExoOauthSmtpAppIdentity -ConfigPath ./config/smtp-app.json -NonInteractive" 'INFO' 
