...
if (-not $SkipAnalyzer) {
    ...
    if ($bad -and $bad.Count -gt 0) { Write-Log "PSScriptAnalyzer found disallowed rules:" 'ERROR'; $bad | Select-Object FileName, Line, RuleName, Severity, Message | Format-Table -AutoSize; exit 1 } else { Write-Log 'No disallowed PSScriptAnalyzer rules found.' 'INFO' }
}

if (-not $SkipGitleaks) {
    Write-Log 'Running gitleaks (repo history) — ensure gitleaks is installed and on PATH...' 'INFO'
    $report = 'gitleaks-prerun-report.json'
    try { & gitleaks detect --redact --report-format json --report-path $report --log-opts "--all"; if (Test-Path $report) { $content = Get-Content -Path $report -Raw; if ($content.Trim()) { $json = $content | ConvertFrom-Json; if ($json -and $json.Count -gt 0) { Write-Log "gitleaks found $($json.Count) findings. See $report" 'ERROR'; exit 2 } } } Write-Log 'gitleaks found no issues.' 'INFO' } catch { Write-Log "gitleaks execution failed: $($_.Exception.Message)" 'WARN'; Write-Log 'If gitleaks is not available, install it or pass -SkipGitleaks to skip this check.' 'WARN' }
}

Write-Log 'Running unit tests (Pester)...' 'INFO'
try { $res = Invoke-Pester -Script .\Tests\Unit -PassThru -Verbose; if ($res.FailedCount -gt 0) { Write-Log 'Unit tests failed. Fix tests before proceeding.' 'ERROR'; exit 3 } Write-Log 'Unit tests passed.' 'INFO' } catch { Write-Log "Running tests failed: $($_.Exception.Message)" 'ERROR'; exit 3 }
